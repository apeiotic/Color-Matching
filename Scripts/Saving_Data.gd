class_name SavedGame
extends  Resource


#region Level Times
##-------------Saving Time--------------##

#------------level1--------------#
@export var level1_time : String
@export var level1_time2 : String
@export var level1_time3 : String

#------------level2--------------#
@export var level2_time : String
@export var level2_time2 : String
@export var level2_time3 : String

#------------level3--------------#
@export var level3_time : String
@export var level3_time2 : String
@export var level3_time3 : String

#------------level4--------------#.
@export var level4_time : String
@export var level4_time2 : String
@export var level4_time3 : String

#------------level5--------------#.
@export var level5_time : String
@export var level5_time2 : String
@export var level5_time3 : String

#------------level6--------------#.
@export var level6_time : String
@export var level6_time2 : String
@export var level6_time3 : String

@export var level7_time : String
@export var level8_time : String
@export var level9_time : String
@export var level10_time :String
@export var level11_time :String

#endregion


##--------------Finsihed levels --------------##
@export var FinishedLevel1 : bool = false
@export var FinishedLevel2 : bool = false
@export var FinishedLevel3 : bool= false
@export var FinishedLevel4 : bool= false
@export var FinishedLevel5 : bool= false
@export var FinishedLevel6 : bool= false
@export var FinishedLevel7 : bool= false
@export var FinishedLevel8 : bool= false
@export var FinishedLevel9 : bool= false
@export var FinishedLevel10 : bool= false


##-------------SETTINGS RELATED-------------##
@export var CameraBob : bool = false
@export var CameraSway : bool = false
@export var CameraShake : bool = false
@export var CameraDrag : bool = false
@export var Senstivity: float = 1.0
